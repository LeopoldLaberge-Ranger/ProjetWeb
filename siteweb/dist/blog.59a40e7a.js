...
function dynamicInsert() {
    fetch("http://localhost:3000/blogs").then((response)=>response.json()).then((json)=>{
        ...
        blogs.forEach((blog, index)=>{
            let card = `<div id="card${index}" class="card text-white bg-secondary shadow-lg" style="width: 18rem;">
          ...
          <a href="#" class="btn btn-info">Visiter ${blog.title}</a>
          </div>
          </div>`;
            const card = document.getElementById(`cards`).innerHTML += card;
        });
...
},{"bootstrap":"h36JB"}]},["9EN09","8lRBv"], "8lRBv", "parcelRequirec9d7")

//# sourceMappingURL=blog.59a40e7a.js.map
